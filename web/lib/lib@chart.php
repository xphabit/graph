...
public static function create($config = array()) {
	$chart = prop::text($config, 'chart');
	$width = self::size($config, 'width');
	$height = self::size($config, 'height');
	$image = @imagecreatetruecolor($width, $height);
	call_user_func(array('self', $chart), $image, $config);
	$type = prop::text($config, 'type');
	call_user_func(array('self', $type), 
		$image, $config);
}

// 检查高度和宽度
private static function size(&$config, $proptery) {
	$value = prop::number($config, $proptery);
	if ($value == 0) {
		$config[$proptery] = 100;
		$value = 100;
	}
...
// 绘制圆环
private static function doughnut($image, $config) {
	imageantialias($image, false);
	imagealphablending($image, true);
	$width = prop::number($config, 'width');
	$height = prop::number($config, 'height');
	$white = imagecolorallocate($image, 255, 255, 255);
	$pointY = ceil($height / 2);
	$pointX = ceil($width / 2);
	imagefilledrectangle($image, 0, 0, $width, $height, $white);
	$value = prop::number($config, 'value') / 50 * M_PI;
	self::imageSmoothArc($image, $pointX, $pointY, $width-5, $height-5, 
		array(105, 146, 165, 0), 0.1, $value-0.1);
	self::imageSmoothArc($image, $pointX, $pointY, $width-5, 
		$height-5, array(255, 127, 39, 0), $value, 2 * M_PI);
	self::imageSmoothArc($image, $pointX, $pointY, $width - 15, $height - 15, 
		array(255, 255, 255, 0), 0, 2 * M_PI);
}

// 输出为图形
private static function png($image, $config) {
	$file = prop::text($config, 'file');
	if ($file != '') {
		imagepng($image, $file);
	} else {
...
http://www.ulrichmierendorff.com/software/antialiased_arcs.html#toc1
*/
private static function imageSmoothArcDrawSegment (&$img, $cx, $cy, $a, $b, $aaAngleX, $aaAngleY, $color, $start, $stop, $seg)
{
...
private static function imageSmoothArc ( &$img, $cx, $cy, $w, $h, $color, $start, $stop)
{
    // Originally written from scratch by Ulrich Mierendorff, 06/2006
    // Rewritten and improved, 04/2007, 07/2007
    ...
    for ($i=0; $i<4;$i++)
    {
        if ($start < ($i+1)*M_PI/2)
        {
            if ($start > $i*M_PI/2)
            {
                if ($stop > ($i+1)*M_PI/2)
                {
                    self::imageSmoothArcDrawSegment($img, $cx, $cy, $a, $b, $aaAngleX, $aaAngleY , $color, $start, ($i+1)*M_PI/2, $i);
                }
                else
                {
                    self::imageSmoothArcDrawSegment($img, $cx, $cy, $a, $b, $aaAngleX, $aaAngleY, $color, $start, $stop, $i);
                    break;
                }
            }
            else
            {
                if ($stop > ($i+1)*M_PI/2)
                {
                    self::imageSmoothArcDrawSegment($img, $cx, $cy, $a, $b, $aaAngleX, $aaAngleY, $color, $i*M_PI/2, ($i+1)*M_PI/2, $i);
                }
                else
                {
                    self::imageSmoothArcDrawSegment($img, $cx, $cy, $a, $b, $aaAngleX, $aaAngleY, $color, $i*M_PI/2, $stop, $i);
                    break;
                }
            }
        }
