/**
http://localhost:8080/g/index.php?id=fd522fc0-9fde-457f-f355-c2fca6e85c71&v=0
https://demo.stimulsoft.com/demo_js_2019-2-2/designer.html?reportname=PanelsSideBySideLists&themename=Purple&localization=zh
...
EditorUi = function(editor, container, lightbox)
{
	...
	// Resolve component
	var resolve = sys.Pod.x;
	this.footbar = resolve(box.Footbar);
	this.edgebar = resolve(box.Edgebar);
...
/**
 * Specifies the width of the format panel. Default is 265.
 */
EditorUi.prototype.formatWidth = 265;
...
{
	this.formatWidth = (forceHide || this.formatWidth > 34) ? 34 : 265;
	this.refresh();
	this.format.refresh();
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	this.formatInnerContainer.style.display = fw <= 34 ? 'none' : '';
	//this.edgeContainer.style.borderRight = fw == 34 ? '0px' : '1px solid rgb(192, 192, 192)';
...
EditorUi.prototype.showLayer = function(value)
{
	if(this.formatWidth > 34) {
	if(value) {
		if(this.formatInnerContainer != null)
			this.formatInnerContainer.style.display = "none";
		if(this.layerInnerContainer != null)
			this.layerInnerContainer.style.display = "block";
		this.layersWindow.redresh();
	} else {
		if(this.formatInnerContainer != null)
			this.formatInnerContainer.style.display = "block";
		if(this.layerInnerContainer != null)
			this.layerInnerContainer.style.display = "none";
	}
	}
}
...
EditorUi.prototype.createDivs = function()
{
	...
	this.edgeContainer = this.edgebar.e(resolve);
	this.formatContainer.appendChild(this.edgeContainer);
	this.layerInnerContainer = this.createDiv('geLayerContainer');
	this.layerInnerContainer.style.display = "none";
	this.layerInnerContainer.style.position = "relative";
	this.layerInnerContainer.style.overflow = "hidden";
	this.layerInnerContainer.style.height = "100%";

	this.formatInnerContainer = this.createDiv('geFormatContainer');
	this.formatInnerContainer.style.height = '100%';
	//this.formatInnerContainer.style.borderLeft = '1px solid #fff';
	this.formatContainer.appendChild(this.formatInnerContainer);
	this.formatContainer.appendChild(this.layerInnerContainer);

	this.diagramContainer = this.createDiv('geDiagramContainer');
	this.footerContainer = this.footbar.f(resolve);//uiControl_element
...
EditorUi.prototype.createUi = function()
{
	...
	this.format = (this.editor.chromeless || !this.formatEnabled) ? null : this.createFormat(this.formatInnerContainer);
	...
	if(this.layerInnerContainer != null) {
		this.layersWindow = new LayersWindow(this, document.body.offsetWidth - 280, 120, 220, 180);
		this.layerInnerContainer.appendChild(this.layersWindow.contentDiv);
	}
};
	...
	if (name != null)
	{
		...
		try
		{
			...
			else
			{
				...
				{
					$id = null;
					if(this.editor.graph.graphId)
						$id = this.editor.graph.graphId;
					var r = new mxXmlRequest(SAVE_URL, ($id != null?"data@id="+$id+"&":"") +'ajax=graph_save&data@filename=' + encodeURIComponent(name) +
						'&data@xml=' + encodeURIComponent(xml), 'POST', false);
					r.send(mxUtils.bind(this, function(e){
						if(!this.editor.graph.graphId)
							this.editor.graph.graphId = eval("(" + e.request.responseText + ")").id;
						}));
				}
				else
				{
...
{
	var d = sys.Pod.x/*sysPod_depend*/("link");
	d.t/*uiControl_update*/({c:fn});
	d.$02(value);/*display*/
};
