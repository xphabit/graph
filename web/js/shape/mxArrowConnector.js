...
mxArrowConnector.prototype.paintEdgeShape = function(c, pts)
{
	var _this = this._;
	// Geometry of arrow
	var strokeWidth = _this.strokewidth;
	
	if (_this.outline)
	{
		strokeWidth = Math.max(1, mxUtils.getNumber(_this.style, mxConstants.STYLE_STROKEWIDTH, _this.strokewidth));
	}
	
	var startWidth = _this.getStartArrowWidth() + strokeWidth;
	var endWidth = _this.getEndArrowWidth() + strokeWidth;
	var edgeWidth = _this.outline ? _this.getEdgeWidth() + strokeWidth : _this.getEdgeWidth();
	var openEnded = _this.isOpenEnded();
	var markerStart = _this.isMarkerStart();
	var markerEnd = _this.isMarkerEnd();
	var spacing = (openEnded) ? 0 : _this.arrowSpacing + strokeWidth / 2;
	var startSize = _this.startSize + strokeWidth;
	var endSize = _this.endSize + strokeWidth;
	var isRounded = _this.isArrowRounded();
	...
	{
		_this.paintMarker(c, pts[0].x, pts[0].y, nx, ny, startSize, startWidth, edgeWidth, spacing, true);
	}
	else
	{
	...
	{
		_this.paintMarker(c, pe.x, pe.y, -nx, -ny, endSize, endWidth, edgeWidth, spacing, false);
	}
	else
	{
	...
	if (pts.length > 2)
	{
		...
		{
			c.begin();
			_this.paintMarker(c, pts[0].x, pts[0].y, startNx, startNy, startSize, startWidth, edgeWidth, spacing, true);
			c.stroke();
			c.end();
		}
		...
		{
			c.begin();
			_this.paintMarker(c, pe.x, pe.y, -nx, -ny, endSize, endWidth, edgeWidth, spacing, true);
			c.stroke();
			c.end();
		}
	}
