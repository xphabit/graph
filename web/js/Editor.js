...
function Dialog(editorUi, elt, w, h, modal, closable, onClose, noScroll, transparent)
{
	...
	if (closable)
	{
		...
		img.className = '_xi_close geDialogClose';
		img.style.left = (left + w+ 40- dx) + 'px';
	...
	mxGraphView.prototype.validateBackgroundStyles = function()
	{
		...
		if (graph.isGridEnabled())
		{
			...
			if (mxClient.IS_SVG)
			{
				...
				image = 'url(' + 'data:image/svg+xml;base64,' + window.btoa(image) + ')'
		...
		{
			if(!graph.container.classList.contains('geDiagramContainer'))
				graph.container.classList.add('geDiagramContainer');
